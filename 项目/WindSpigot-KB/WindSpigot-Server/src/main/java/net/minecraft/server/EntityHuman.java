...
public abstract class EntityHuman extends EntityLiving {
	...
	public void attack(Entity entity) {
		if (entity.aD()) {
			if (!entity.l(this)) {
				...
				// WindSpigot start - 无敌帧击退标记（damageEntity 被无敌帧拦截但已施加阶段一击退）
				boolean iframeKb = entity instanceof EntityLiving && ((EntityLiving) entity).kbIframeKbApplied;
				if (entity instanceof EntityLiving) {
					((EntityLiving) entity).kbIframeKbApplied = false;
				}
				// WindSpigot end

				if (flag2 || iframeKb) { // WindSpigot - 无敌帧击退同样走击退流程
				if (i > 0 || KnockbackEngine.isSprintingEffective(this)) {
					KnockbackProfile profile = (entity.getKnockbackProfile() == null)
							? KnockbackConfig.getCurrentKb()
							: entity.getKnockbackProfile();

					// WindSpigot start - 阶段二击退：疾跑绝对值累加 + 击退附魔按等级叠加
					// （引擎内处理疾跑宽松判定、对刀独立参数、动态misplay补偿）
					KnockbackEngine.applySprintKnockback(this, entity, EnchantmentManager.a(this), profile);
					// WindSpigot end
						this.motX *= 0.6D;
						this.motZ *= 0.6D;
	...
	// WindSpigot start - 疾跑宽限记录（疾跑击退宽松判定用）
	/** 最后一次收到 STOP_SPRINTING 包的 tick（由 PlayerConnection 更新） */
	public int kbLastSprintStopTick = Integer.MIN_VALUE;
	// WindSpigot end
