...
function getParamOffset(param) {
    ...
    if (param.indexOf('multiplier.') !== -1) return 0.03;
...
// 理论范围下限允许负数的参数(如超出限高后的垂直击退可以 ≤ 0)
function paramAllowsNegative(param) {
    return param === 'y-limit.vertical-kb-after-limit';
}
...
function saveCandidates() {
    try {
        localStorage.setItem('kbcore_candidates', JSON.stringify(candidates));
    } catch (e) { /* 忽略存储错误 */ }
}
...
function autoSaveState() {
    ...
    try {
        localStorage.setItem('kbcore_auto_save', JSON.stringify(state));
    } catch (e) { /* 忽略存储错误 */ }
}

function autoRestoreState() {
    try {
        var raw = localStorage.getItem('kbcore_auto_save');
        if (!raw) return false;
        var state = JSON.parse(raw);
        if (state.version !== '2.0') return false;
        ...
        if (state.presets) {
            ...
            localStorage.setItem('kbcore_presets', JSON.stringify(presets));
        }
        if (state.bools) {
            for (var b in BOOLS) {
                if (typeof state.bools[b] === 'boolean') BOOLS[b] = state.bools[b];
            }
            renderBoolPanel();
        }
        // 迁移:确保新旧存档都包含全部可调参数
...
function cmpPreview() {
    ...
    var lines = [];
    lines.push('# knockback.yml 组合配置（来自保留候选）');
    ...
    document.getElementById('cmpPreview').value = kbmSignContent(lines.join('\n'));
}
...
function cmpExport() {
    ...
    var link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
		link.download = 'knockback.yml'; // 直接导出为内核配置文件名，放入服务端根目录后 /kb reload 生效
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(link.href);
...
window.onload = function() {
    ...
    if (restored) {
        ...
        for (var b in BOOLS) {
            BOOLS[b] = (b === 'pvp.enabled' || b === 'stop-sprint' || b === 'y-limit.enabled');
        }
        renderBoolPanel();
        renderAll();
        enableButtons(false);
