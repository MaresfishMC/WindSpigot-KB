'use strict';
// 端内复核: /ping 文本编码修复 + 击退参数未回归
const { spawn } = require('child_process');
const fs = require('fs');
const DIR = 'F:\\open\\新服务器\\senven (2)';
const LOG = 'F:\\open\\新服务器\\PVP内核\\分析\\server_text.log';
...
function onData(d) {
  ...
  if (/KBProbe|Done \(|ERROR|Exception/i.test(d)) process.stdout.write(d);
...
(async () => {
  ...
  await sleep(2500);
  console.log('\n########## /ping 文本编码 ##########');
  await send('kbprobe text', 1800);
  console.log('\n########## 击退参数未回归 ##########');
  await send('kbprobe', 1600);
  await send('kb get base-kb.horizontal-limit', 900);
