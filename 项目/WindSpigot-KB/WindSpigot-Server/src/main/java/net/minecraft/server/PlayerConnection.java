...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
	...
	public void a(PacketPlayInEntityAction packetplayinentityaction) {
		...
		case 4:
			...
			this.player.kbLastSprintStopTick = MinecraftServer.currentTick; // WindSpigot - 疾跑宽限记录
			break;
	...
	public void a(PacketPlayInUseEntity packetplayinuseentity) {
		...
		if (entity != null) {
			...
			// WindSpigot start - 疾跑宽松判定：疾跑(含宽限)时放宽攻击距离
			if (KnockbackEngineSettings.b("sprint-reach.enabled")
					&& KnockbackEngine.isSprintingEffective(this.player)) {
				double extra = KnockbackEngineSettings.d("sprint-reach.extra");
				if (extra > 0.0D) {
					double reach = Math.sqrt(d0) + extra;
					d0 = reach * reach;
				}
			}
			// WindSpigot end
