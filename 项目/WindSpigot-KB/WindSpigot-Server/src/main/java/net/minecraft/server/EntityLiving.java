...
import com.windpvp.windspigot.knockback.KnockbackEngine;
...
public abstract class EntityLiving extends Entity {
	...
	// WindSpigot start - KB引擎扩展字段
	/** 击退滞空期间使用自定义重力（落地自动解除） */
		...
		if (this.isInvulnerable(damagesource)) {
			...
			this.maxNoDamageTicks = KnockbackEngineSettings.i("hit-delay");
				...
				if (this.noDamageTicks > this.maxNoDamageTicks / 2.0F) {
					...
					if (f <= this.lastDamage || !KnockbackEngineSettings.b("damage-increment")) {
						...
						this.applyIframeKnockback(damagesource); // WindSpigot - 无敌帧仍击退
						return false;
					}
					// WindSpigot end
					...
					// CraftBukkit end
					this.lastDamage = f;
					this.applyIframeKnockback(damagesource); // WindSpigot - 无敌帧仍击退(伤害差值分支)
					flag = false;
				} else {
					// CraftBukkit start
	...
	// WindSpigot start - 无敌帧击退（iframe-knockback）
	/**
	 * 无敌帧内仍施加击退：不造成伤害，仅按相对位置计算击退并标记，
	 * 玩家受害者的速度包同步由 EntityHuman.attack() 负责（读取 kbIframeKbApplied）。
	 */
	private void applyIframeKnockback(DamageSource damagesource) {
		...
		// 玩家受害者的速度同步依赖 EntityHuman.attack() 流程，仅人类攻击者时启用，
		// 避免生物攻击造成的击退包无法送达而产生不同步
		if (this instanceof EntityPlayer && !(entity instanceof EntityHuman)) {
			return;
		}
		double distanceX = entity.locX - this.locX;
		double distanceZ;
		for (distanceZ = entity.locZ - this.locZ; distanceX * distanceX
				+ distanceZ * distanceZ < 1.0E-4D; distanceZ = (random.nextDouble() - random.nextDouble()) * 0.01D) {
			distanceX = (random.nextDouble() - random.nextDouble()) * 0.01D;
		}
		this.a(distanceX, distanceZ, damagesource);
		this.kbIframeKbApplied = true;
	}
	// WindSpigot end
	...
	public void a(double x, double z, DamageSource source) {
		if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
			...
			// WindSpigot start - 新版击退引擎：玩家近战走 KnockbackEngine
			// （空中/地面分离、对刀独立乘区、距离衰减、连击递增、防飞天、动量保留）
			if (!(source instanceof EntityDamageSourceIndirect)) {
				Entity damager = source instanceof EntityDamageSource ? ((EntityDamageSource) source).getEntity()
						: null;
				if (damager instanceof EntityHuman) {
					KnockbackEngine.applyBaseKnockback(this, x, z, (EntityHuman) damager);
					return;
				}
			}
			// WindSpigot end
			...
			// WindSpigot start - 疾跑击退宽松判定
			if (kb.isSprintLenientEnabled() && source instanceof EntityDamageSource) {
				EntityDamageSource entitySource = (EntityDamageSource) source;
				if (entitySource.getEntity() instanceof EntityHuman) {
					EntityHuman attacker = (EntityHuman) entitySource.getEntity();
					if (attacker.isSprinting()) {
						// 应用疾跑击退系数
						horizontal *= kb.getSprintHorizontalMultiplier();
						vertical *= kb.getSprintVerticalMultiplier();
					}
				}
			}
			// WindSpigot end

			// WindSpigot start - 空中/地面分开判定
			if (this.onGround) {
				// 地面击退
				horizontal *= kb.getGroundHorizontalMultiplier();
				vertical *= kb.getGroundVerticalMultiplier();
			} else {
				// 空中击退
				horizontal *= kb.getAirHorizontalMultiplier();
				vertical *= kb.getAirVerticalMultiplier();
			}
			// WindSpigot end
